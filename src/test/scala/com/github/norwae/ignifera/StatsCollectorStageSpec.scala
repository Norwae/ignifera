...
class StatsCollectorStageSpec extends FlatSpec with Matchers with BeforeAndAfterEach with BeforeAndAfterAll with Eventually {
  ...
  "The stats collector" should "track requests in flight" in {
    ...
    eventually {
      ...
      val processingTimeSample = CollectorRegistry.defaultRegistry.getSampleValue("http_request_duration_microseconds", Array("method", "status", "quantile"), Array("GET", "200", "0.95"))
  ...
  it should "track request sizes" in {
    ...
    eventually {
      val sample = CollectorRegistry.defaultRegistry.getSampleValue("http_request_size_bytes", Array("method", "status", "quantile"), Array("POST", "200", "0.95"))
  ...
  it should "track response sizes" in {
    ...
    eventually {
      val sample = CollectorRegistry.defaultRegistry.getSampleValue("http_response_size_bytes", Array("method", "status", "quantile"), Array("PUT", "200", "0.95"))
      sample should not be null
      sample.intValue() should (be >= 1000 and be < 1100)
    }
  }
